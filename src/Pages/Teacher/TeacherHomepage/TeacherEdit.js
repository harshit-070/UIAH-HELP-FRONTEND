...
class TeacherPage extends Component {
  state = {
    ...
    Form: {
      ...
      willLearn: {
        ...
        rows: "5",
        cols: "70",
        placeholder: "students will learnt to apply Mathematics skills...",
        value: "",
        validation: {
          required: true,
          minLength: 1,
  ...
  render() {
    ...
    if (this.state.Form.category["Physics"]) {
      classWeb = ["ButtonClicked"];
      //console.log("clicked11",classWeb.join(' '),this.state.Form.category['Physics'])
    } else classWeb = [];

    if (this.state.Form.category["Chemistry"]) {
      classWebDesign = ["ButtonClicked"];
    } else classWebDesign = [];

    if (this.state.Form.category["Aptitude"]) {
      classML = ["ButtonClicked"];
    } else classML = [];

    if (this.state.Form.category["Mathematics"]) {
      classReact = ["ButtonClicked"];
    } else classReact = [];

    if (this.state.Form.category["Biology"]) {
      classBiology = ["ButtonClicked"];
    } else classBiology = [];

    if (this.state.Form.category["Logical_Reasoning"]) {
